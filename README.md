...
| Basic | ggml-base.bin |
| Basic English | ggml-base.en.bin |
| Medium | ggml-small.bin |
| Medium English | ggml-small.en.bin |
| High | ggml-medium.bin |
| High English | ggml-medium.en.bin |
...
- Installed version of VEGAS Pro (provides the `ScriptPortal.Vegas.dll`)

```powershell
# From repo root with Powershell as administrator
.\scripts\build.ps1
```

Output: `dist\AkkeohsSubtitlesSetup.exe` (single file to ship).

Without VEGAS installed, the build uses a **compile stub** for `ScriptPortal.Vegas.dll`. Before shipping, rebuild with the real DLL from your VEGAS install (for example `C:\Program Files\VEGAS\VEGAS Pro XX.X\ScriptPortal.Vegas.dll`).
